...
////// Lighting:
                ...
                float3 finalColor = lerp(((SHLight( normalDirection )*_TexMap_var.rgb)+(_TexMap_var.rgb*max(0,dot(normalDirection,lightDirection))*_LightColor0.rgb*attenuation)+(pow(max(0,dot(halfDirection,normalDirection)),100.0)*_SpecMap_var.rgb*_LightColor0.rgb*attenuation)),texCUBE(_CubeMap,viewReflectDirection).rgb,pow((1.0 - saturate(dot(normalDirection,viewDirection))),5.0));
